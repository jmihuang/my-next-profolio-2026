...
function ensureFeaturedColumn() {
  ...
  if (columns.some((column) => column.name === "featured_order")) return;

  db.exec("ALTER TABLE products ADD COLUMN featured_order INTEGER");
  db.prepare("UPDATE products SET featured_order = CASE slug WHEN 'cleaning-doctor' THEN 1 WHEN 'putien-interior-design' THEN 2 END WHERE slug IN ('cleaning-doctor', 'putien-interior-design')").run();
}
...
export function getFeaturedProducts() {
  const rows = db
    .prepare("SELECT * FROM products WHERE status = 'published' AND featured_order IS NOT NULL ORDER BY featured_order ASC, id ASC LIMIT 2")
    .all();
  return rows.map((row) => toProduct(row, getGallery(row.id), getSections(row.id)));
}
...
function nextSortOrder() {
  return db.prepare("SELECT COALESCE(MAX(sort_order), -1) + 1 AS value FROM products").get().value;
}
...
export function moveProduct(id, direction) {
  const product = getProductById(id);
  if (!product) throw new Error("Product not found.");
  const comparison = direction === "up" ? "<" : ">";
  const order = direction === "up" ? "DESC" : "ASC";
  const adjacent = db.prepare(`SELECT * FROM products WHERE sort_order ${comparison} ? ORDER BY sort_order ${order}, id ${order} LIMIT 1`).get(product.sort_order);
  if (!adjacent) return product;
  db.transaction(() => {
    db.prepare("UPDATE products SET sort_order = ? WHERE id = ?").run(adjacent.sort_order, product.id);
    db.prepare("UPDATE products SET sort_order = ? WHERE id = ?").run(product.sort_order, adjacent.id);
  })();
  return getProductById(id);
}

export function toggleFeaturedProduct(id) {
  const product = getProductById(id);
  if (!product) throw new Error("Product not found.");

  if (product.featured_order !== null && product.featured_order !== undefined) {
    db.prepare("UPDATE products SET featured_order = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(id);
    return getProductById(id);
  }

  const featuredCount = db.prepare("SELECT COUNT(*) AS value FROM products WHERE featured_order IS NOT NULL").get().value;
  if (featuredCount >= 2) throw new Error("首頁最多只能置頂兩個作品，請先取消其中一個。");

  db.prepare("UPDATE products SET featured_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").run(featuredCount + 1, id);
  return getProductById(id);
}
