...
export async function getMoreWorkProducts() {
  return Promise.all((await all("SELECT * FROM products WHERE status = 'published' OR show_in_confidential_portfolio = 1 ORDER BY sort_order ASC, id ASC")).map(hydrateProduct));
}
