...
export async function createProduct(input) {
  ...
  const result = await run("INSERT INTO products (slug, title, eyebrow, role, summary, card_technology_line, categories_json, technologies_json, notes_json, cover_image_path, case_study_key, status, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", product.slug, product.title, product.eyebrow, product.role, product.summary, product.cardTechnologyLine, product.categoriesJson, product.technologiesJson, product.notesJson, product.coverImagePath, product.caseStudyKey, product.status, product.sortOrder);
...
export async function updateProduct(id, input) {
  ...
  if (!product.coverImagePath.startsWith("/")) throw new Error("Cover image path is required.");
  await run("UPDATE products SET title = ?, eyebrow = ?, role = ?, summary = ?, card_technology_line = ?, categories_json = ?, technologies_json = ?, notes_json = ?, cover_image_path = ?, case_study_key = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", product.title, product.eyebrow, product.role, product.summary, product.cardTechnologyLine, product.categoriesJson, product.technologiesJson, product.notesJson, product.coverImagePath, product.caseStudyKey, product.status, id);
